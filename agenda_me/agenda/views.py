...
class CreateView(generics.ListCreateAPIView):
    ...
    def post(self, request: Request, *args, **kwargs):
        ...
        repeat_weekday: Union[list[int], None] = (
            request.data.get("repeat_weekday", None)
            if type(request.data) is dict
            else request.data.getlist("repeat_weekday", None)
        )

        change_dict_keyname: Callable[[dict, str, str]] = lambda d, old_k, new_k: {
            new_k if k == old_k else k: v for k, v in d.items()
        }
        ...
        if must_repeat is True:
            ...
            from_datetime_to_time = lambda datetime: datetime.split("T")[1].replace(
                "Z", ""
            )
            ...
            agenda_data.update(
                {
                    "time_init": from_datetime_to_time(
                        agenda_data.pop("date_init", None)
                    ),
                    "time_end": from_datetime_to_time(
                        agenda_data.pop("date_end", None)
                    ),
            ...
            agenda_data = {
                "sala_id" if k == "sala" else k: v for k, v in agenda_data.items()
            }
            agenda_data = {
                "creator_department_id" if k == "creator_department" else k: v
                for k, v in agenda_data.items()
            }
        ...
        try:
            if code == agenda_instance.code:
                ...
                if must_repeat is False:
                    ...
                    periodic_item: Union[
                        PeriodicAgenda, None
                    ] = PeriodicAgenda.objects.filter(code=agenda_instance.code).first()
                    if periodic_item is not None:
                        ...
                        response_data[
                            "message"
                        ] = "Agendamento periódico não encontrado."

                    # desativa a repetição das outras agendas iguais
                    other_same_agendas: QuerySet[Agenda] = Agenda.objects.filter(
                        code=agenda_instance.code
                    )
            ...
            raise ValidationError(
                detail={"detail": "Erro interno no servidor."},
...
class RefreshAgendaView(views.APIView):
    ...
    def _check_periodic_items(
        self, request: Request, periodic_items: QuerySet[PeriodicAgenda]
    ):
        """
        ...
        if sala_id is not None:
            ...
            for periodic_agenda in agendas:
                ...
                for weekday in repeat_weekdays:
                    ...
                    this_monday = today_00_00 + timedelta(
                        days=0 - today_weekday
                    )  # segunda-feira, 00h00
                    this_sunday = today_23_59 + timedelta(
                        days=6 - today_weekday
                    )  # domingo, 23h59

                    # Se já houver uma reunião nesta semana com as mesmas informações, não cria outra.
                    already_created_on_this_week = bool(
                        Agenda.objects.filter(
                            date_end__gte=this_monday,
                            date_end__lte=this_sunday,
                            **agenda_fields,
                        )
                    )
    ...
    def get(self, request: Request):
        ...
        if bool(periodic_items) is False:
            return Response({"message": "Não há agendas periódicas."})
        ...
        try:
            ...
            return Response(
                {
                    ...
                    else "Nenhuma agenda precisou ser criada.",
                },
            ...
            print(e)
            raise ValidationError(
                detail={"message": "Erro ao atualizar agenda desta semana."},
                code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
